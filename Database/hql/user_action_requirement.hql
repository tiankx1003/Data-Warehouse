...
select ud.mid_id,ud.os,'2019-11-07' dt
from 
	dws_uv_day ud 
	left join dws_new_mid_day nm
	on ud.mid_id=nm.mid_id
where ud.dt='2019-11-07' and nm.mid_id is null; -- 通过left join去除已有的即为新增的
  -- 验证
...
partition(dt)
select 
	ud.mid_id,nm.create_date,1 retention_day,'2019-11-07' dt
from 
	dws_uv_day ud 
	join dws_new_mid_day nm
	on ud.mid_id=nm.mid_id
where ud.dt='2019-11-07' and nm.create_date=date_add('2019-11-07',-1);
...
union all 
select 
	ud.mid_id,nm.create_date,2 retention_day,'2019-11-07' dt
from 
	dws_uv_day ud 
	join dws_new_mid_day nm
	on ud.mid_id=nm.mid_id
where ud.dt='2019-11-07' and nm.create_date=date_add('2019-11-07',-2)
union all
select 
	ud.mid_id,nm.create_date,3 retention_day,'2019-11-07' dt
from 
	dws_uv_day ud 
	join dws_new_mid_day nm
	on ud.mid_id=nm.mid_id
where ud.dt='2019-11-07' and nm.create_date=date_add('2019-11-07',-3)
...
from 
	...
	where dt<='2019-11-07'
	group by mid_id
	having count(*)=1 and max(dt)<date_add('2019-11-07',-7)) t1;
...
select 
	'209-08-28' dt,
	concat(date_add(next_day('2019-11-07','MO'),-7),'_',date_add(next_day('2019-11-07','MO'),-1)) wk_dt,
	count(*) back_count
from 
	(
		select t1.mid_id
		...
		(
			select mid_id
			from dws_uv_wk
			where wk_dt=concat(date_add(next_day('2019-11-07','MO'),-7),'_',date_add(next_day('2019-11-07','MO'),-1))
		) t1
		left join 
		(
			select mid_id
			from dws_new_mid_day
			where create_date between date_add(next_day('2019-11-07','MO'),-7) and date_add(next_day('2019-11-07','MO'),-1)
		) t2
		...
		(
			select mid_id
			from dws_uv_wk
			where wk_dt=concat(date_add(next_day('2019-11-07','MO'),-14),'_',date_add(next_day('2019-11-07','MO'),-7-1))
		) t3
...
from 
	(
		...
		from dws_uv_day
		group by mid_id
		having max(dt)<=date_add('2019-11-07',-7)
	) t1;
...
select 
	'2019-11-07' dt,
	concat_ws(date_add('2019-11-07',-21),'_',date_add('2019-11-07',-1)) wk_dt,
	count(*) continuity
from 
	(
		select mid_id
		from dws_uv_wk
		where wk_dt>=concat_ws(date_add('2019-11-07',-21),'_',date_add('2019-11-07',-14-1)) and 
			wk_dt<=concat_ws(date_add('2019-11-07',-7),'_',date_add('2019-11-07',-1))
		group by mid_id
		having count(*)=3
	) t1;
...
select 
	'2019-11-07' dt,
	concat_ws(date_add('2019-11-07',-6),'_','2019-11-07') wk_dt,
					...
					(
						...
						from 
							(
								...
								from dws_uv_day
								where dt between date_add('2019-11-07',-6) and '2019-11-07'
							) t1
					) t2
